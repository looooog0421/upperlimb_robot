...
def normalize(d_n,d_max = 10):
    ...
    # print(extra.shape)
    d_extra = np.vstack((extra, d_select)) # d_extra 中包含了极值点以及极值点对应的序列数
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(data_dir, "stiffness_data1.txt")
    
    d_n,dn_real = stiff(ta=40,N=3)
    # # # print(d_n.shape)
    ...
    for i in range(d_n.shape[0]):
            with open(data_path, 'a') as f:
                # writer = csv.writer(f)
                # writer.writerow(d_n[i])
                for j in range(d_n.shape[1]):
                    f.write("{} ".format(d_n[i,j]))
                f.write("\n")
                # f.write('\n')
